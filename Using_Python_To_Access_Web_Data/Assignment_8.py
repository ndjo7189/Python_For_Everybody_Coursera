...
# contact a web service and retrieve JSON for the web service and parse that data, and 
# retrieve the first place_id from the JSON. A place ID is a textual identifier that 
...
# This API uses the same parameters (sensor and address) as the Google API. This API also has 
# no rate limit so you can test as often as you like. If you visit the URL with no parameters, 
# you get a list of all of the address values which can be used with this API.
# To call the API, you need to provide a sensor=false parameter and the address that you are 
# requesting as the address= parameter that is properly URL encoded using the urllib.urlencode() 
...
# code below. Hint: The first seven characters of the place_id are "ChIJLwP ..."
...
while True:
    ...
    #print json.dumps(js, indent=4)[:1200]
